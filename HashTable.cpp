...
void HashTable::Insert(Bid bid) {
	...
	Node* oldNod = &(nodes.at(key));

	if(oldNod != nullptr) {
		// Node found
		if (oldNod->_key == UINT_MAX) {
			oldNod->_key = key;
			oldNod->_bid = bid;
			oldNod->next = nullptr;
		} else {
			...
			while(true) {
				if(oldNod->next == nullptr) {
					break;
				}
				oldNod = oldNod->next;
			}
			oldNod->next = new Node(key, bid);
		}

	} else {
		Node* newNod = new Node(key,bid);
		nodes.insert(nodes.begin() + key, *newNod);
	}
}
...
Bid HashTable::Search(string bidId) {
    ...
    // if no node exist (fail fast condition)
    ...
    // teverse thru node to find the match
    while(true) {
    	if (node == nullptr) {
    		break;
    	}
